...
- **2026-03-17**: **分享卡片 Canvas 绘制功能** ✅ 完成
  - **功能描述**: 将分享卡片从 html2canvas DOM 截图改为 Canvas API 直接绘制
  - **性能提升**: 截图时间从 800ms-1500ms 降至 50-100ms（提升 10-15 倍）
  - **核心特性**:
    1. 纯白圆角卡片（24px 圆角）
    2. 衬线字体（Noto Serif SC / SimSun）
    3. 绿色主调 #2d5a27，橙色强调 #e67e22
    4. 时长数字与单位左右分布
    5. 文案垂直居中，保留用户手动换行和空行
    6. 统计数据三列等宽
    7. 默认头像为绿色圆形
    8. 品牌名与签名同行右侧
    9. 高清输出 scale: 2
    10. 降级方案：保留 html2canvas 作为 Canvas 失败时降级
  - **交互优化**:
    1. 点击卡片放大到 max-w-lg，毛玻璃效果背景
    2. 长文案时内部滚动，按钮悬浮固定
    3. 截图时展开全部内容，保留圆角
  - **技术实现**:
    - `lib/share-card-canvas.ts`: Canvas 绘制工具
    - `app/practice/page.tsx`: 分享卡片组件和导出逻辑
  - **头像加载**: 3 秒超时，超时后降级为绿色默认头像
