...
- **2026-02-21**: **历史练习数据校准功能** - 让用户设置历史练习数据作为统计基础
  - **背景**: 很多用户已有长期练习历史（如练习100天），但使用APP后要从零开始记录，感觉"很亏"
  - **核心功能**:
    - ✅ 在设置 → 个人资料页面添加历史数据校准区域
    - ✅ 可设置历史练习天数和历史平均每次时长
    - ✅ 所有统计数据（总天数、总时长、平均值）以此为基础累加
    - ✅ 带 +/- 按钮方便调节（天数±10，时长±5分钟）
    - ✅ 支持直接输入数字
    - ✅ 实时显示预计历史总时长
  - **数据结构修改**:
  ...
  - **统计计算修改**:
    - `totalStats` 计算时添加历史数据
    - 总天数 = 本地记录天数 + 历史天数
    - 总时长 = 本地记录时长 + (历史天数 × 历史平均时长)
    - 平均值 = 总时长 / 总天数
  - **UI修改**:
    - 设置 → 个人资料页面添加历史数据校准区域（在签名下方）
    - 统计卡片"总熬汤天数"下方显示"含历史记录"提示（当设置了历史数据时）
    - 修改文件: `app/practice/page.tsx`
  - **数据同步**:
    - 历史数据作为 profile 的一部分，自动随用户资料同步到云端
  - **产品决策**: 符合"简单"理念，界面简洁，操作直观，不强制用户设置
