...
function AddPracticeModal({
  ...
  const handleCustomPracticeConfirm = (name: string, notes: string) => {
    // REMOVED: console.log('handleCustomPracticeConfirm called with:', name, notes)
    // REMOVED: console.log('onAddOption function:', onAddOption)
    // 调用父组件的 addOption 方法保存到 localStorage
    if (onAddOption) {
      // REMOVED: console.log('calling onAddOption...')
      onAddOption(name, notes)
      // REMOVED: console.log('onAddOption called')
      // 设置选中的类型
      setType(name)
      ...
      // REMOVED: console.log('onAddOption is undefined!')
      setType(name)
      setShowCustomModal(false)
      onChildModalOpen?.(false)
    }
        ...
        <>
          ...
          >
            ...
            <AccountBindingSection
              ...
              onSyncComplete={(data) => {
                // REMOVED: console.log('Sync completed:', data)
                // ⭐ 更新本地 profile（如果云端有更新）
                if (data?.profile) {
                  // REMOVED: console.log('更新本地 profile:', data.profile)
...
export default function AshtangaTracker() {
  ...
  const { syncStatus, lastSyncTime, failedSyncIds, setFailedSyncIds, setLastSyncStatus, resolveConflict, syncStats, autoSync } = useSync(
    ...
    async (data) => {
      // 同步完成后的回调：更新本地数据
      if (data.records) {
        // REMOVED: console.log('🔄 同步完成，更新本地数据...')
