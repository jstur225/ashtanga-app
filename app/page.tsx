    ...
    return (
        <div className="min-h-screen bg-[#F9F5F0] text-[#2D5A27] font-serif selection:bg-[#2D5A27] selection:text-[#F9F5F0]">
            ...
            <nav className="fixed top-0 w-full p-6 z-50 flex justify-between items-center bg-[#F9F5F0]/80 backdrop-blur-sm">
                ...
                <Link href="/practice">
                    <button className="px-6 py-2 rounded-full border border-[#2D5A27] text-[#2D5A27] hover:bg-[#2D5A27] hover:text-white transition-all duration-300 text-sm tracking-wide bg-transparent">
                        进入练习
                    </button>
                </Link>
            </nav>
            ...
            <section className="py-32 px-6 bg-white relative">
                <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
                    <div className="relative">
                        <div className="aspect-[4/5] bg-gray-100 rounded-[32px] overflow-hidden relative shadow-2xl group">
                            <div className="absolute inset-0 bg-gradient-to-br from-[#F0F4EF] to-[#E1E8DE] transition-colors group-hover:from-[#E8EDE7] group-hover:to-[#D1D9CE]" />
            ...
            <section className="py-32 px-6">
                <div className="max-w-4xl mx-auto text-center space-y-6">
                    <Moon className="w-12 h-12 text-[#2D5A27] mx-auto mb-6 opacity-80" />
...
function FeatureItem({ icon: Icon, title, desc }: { icon: any, title: string, desc: string }) {
    return (
        <div className="flex gap-4 items-start group hover:bg-white/50 p-4 rounded-2xl transition-all duration-300 cursor-default">
            <div className="p-3 bg-[#2D5A27]/5 rounded-xl group-hover:bg-[#2D5A27]/10 transition-colors">
                <Icon className="w-6 h-6 text-[#2D5A27]" />
            </div>
            <div>
                <h3 className="text-lg font-medium mb-1 text-[#2D5A27]">{title}</h3>
