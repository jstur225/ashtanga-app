...
## 2026-08-13 - 方式二（OSS 孤儿每日兜底清理）→ 只记录，暂不部署

- 需求：Vercel 定时任务每日扫 OSS，清“传了字节没登记”的孤儿对象（方式一已覆盖“删除照片”类孤儿）。
- 状态：**暂不部署**，仅记录；需要 OSS 凭据 + 服务端清理函数 + Vercel Cron。
- 触发条件：若后续发现 OSS 存储增长明显或出现孤儿，再评估启用。
...
- [x] 新增 `__tests__/oss-delete.test.ts`；typecheck + vitest + `223/223` 小程序测试通过。
- [ ] 部署后确认服务端 OSS AccessKey 有 `oss:DeleteObject` 权限。
- [ ] 方式二：Vercel 定时任务每日兜底清理"传了字节没登记"的孤儿（需 OSS 凭据 + 服务端清理函数）。


## 2026-08-13 - 照片上传提速方案 C ✅ 代码完成，待真机复验

- [x] 保存门改为“字节落地 OSS + 客户端 HEAD 校验”即放开（不再等慢的元数据登记）。
- [x] photo-storage 拆分 `uploadPhotoToOss` / `registerPhotoMetadata`，后台同步路径保留 `uploadPhoto`。
- [x] 表单即时上传：PUT 成功即替换远端地址、换“登记”待办，尾部后台补登记（并发保护不重复）。
- [x] 保存时给“照片已上传，保存后自动同步”轻提示。
- [x] 测试更新 + 新增，`223/223` 通过；包体门禁通过。
- [ ] 真机复验（多图、保存即走、时间轴、断网失败边界）。
- [ ] OSS 孤儿清理脚本（删除照片只软删元数据，OSS 文件字节会残留成孤儿；需 OSS 凭据）。


## 2026-08-12 - 时间轴加载更多 + 照片上传提速 ✅ 已实现，待真机复验/待部署

- [x] tab2 时间轴“滚到底自动加载历史”已实现（对照网页版 `onReachBottom`）：自动加载上一个月、底部三态、最早记录月为边界、切月重置；新增回归测试，`220/220` 通过。
- [x] `POST /api/photos` 独立请求并行化（会员/照片数/记录/OSS 校验 4→1 批），响应体不变；typecheck + 接口测试通过，**未部署**。
- [x] 小程序请求日志新增 `server_timing` 字段，真机日志已带服务端分段耗时。
- [x] 真机日志(3) 确认：0 错误、字体成功 2.44s、同步/照片健康。
- [x] 决定：Vercel 区域暂不迁移。
- [ ] 真机复验时间轴“滚到底加载更早月份”、切月重置、到底提示。
- [ ] 下次发版带上照片接口并行化后，真机复测单张/9 张上传耗时；若仍慢，按备选方向（保存不阻塞 / 并发+批量签名 / 批量登记 / 压缩大图）评估。


## 2026-08-12 - 服务端慢请求定位：Server-Timing + date 索引 ✅ 已部署，待真机复验

- [x] `/api/membership/status`、`/api/stats/today` 加 `Server-Timing` 响应头（`lib/server-timing.ts`），响应体不变，网页版无感知。
- [x] `practice_records(date)` 索引已入 migration 并在生产 Supabase 执行成功。
- [x] cherry-pick 到生产 master `e7d6fc9b` 并推送，Vercel 自动部署完成，线上实测 Server-Timing 生效。
- [x] 小程序请求日志新增 `server_timing` 字段（`weapp/utils/request.js` + request-logging 测试），V3 JSON 可看到服务端分段耗时。
- [x] 字体 `ERR_CACHE_MISS` 确认为微信开发者工具（Windows）缓存小毛病；线上字体 200 / 295,564B / immutable / HIT / CORS 正常，Android 真机此前成功加载。
- [ ] Android 真机复验一轮并导出 V3 JSON，核对 `server_timing`、会员/照片诊断与字体；若真机字体仍失败，再按预案迁国内 OSS/CDN。

**结论：** 慢接口根因 = Vercel 区域（美东 iad1）到 Supabase 的网络往返 + 冷启动，非查询或小程序代码；后续可考虑 Vercel 函数区域迁 `hkg1`/`sin1` 或合并串行查询。


## 2026-08-12 - 小程序完整诊断日志 V3 ✅ 代码完成，待真机导出验收

- [x] 不再只导出数量：`records_snapshot.all` 包含本机工作区全部记录，包括笔记、突破、照片、色阶、创建/更新时间、软删除、教程、同步状态与关联 pending 操作。
- [x] 登录模式额外保留 `guest_records_snapshot`，可判断注册/迁移后游客记录是否残留或遗漏。
- [x] `options_snapshot`、`profile_snapshot`、`annotations_snapshot` 导出全部练习类型/色阶、资料和标注数据。
- [x] `sync_diagnostics` 导出完整待同步 payload、重试次数、最近错误、孤儿操作、全部 100 条同步日志、冲突和失败日志。
- [x] 导出时只读拉取云端全部有效记录、选项、资料和已缓存月份标注，`cloud_diagnostics.merge_comparison` 逐条标出 `local_only / remote_only / divergent / matched`、变化字段、较新一侧及关联操作。
- [x] `photo_diagnostics.all_references` 包含全部照片引用；按网页版相同上限，对最近 12 个唯一照片执行并发健康检查，识别本机文件丢失/0 字节及远端 403/404/0 字节/类型错误/超时。
- [x] `auth_diagnostics` 记录账号模式、会话是否存在、过期时间和用户元数据，但不导出 Token；会员缓存、当前服务端/缓存结果及相关时间线单独成段。
- [x] 网络按接口聚合，页面、登录、照片、同步、会员/支付、字体时间线分别成段；存储只导出键名和估算大小，不导出敏感存储值。
- [x] 密码、验证码、Token、Cookie、支付签名、URL 查询签名和完整邮箱继续递归脱敏。
- [x] 完整小程序自动化测试 `212/212` 通过。
- [ ] 真机导出新 JSON，确认 `_meta.version = 3.0-weapp`、`_meta.diagnostics = full`，并核对照片健康检查和云端合并对照没有超时。

**下一步：** 先制造一个容易辨认的现场（例如编辑一条带照片记录或触发一次同步），随后进入“我的 → 设置 → 数据管理 → 运行日志 → 导出文件”。生成过程现在会进行只读云端和照片检查，通常比 V2 多几秒；单项超过限制会写入 timeout 诊断，但仍会正常导出其余完整内容。

---

## 2026-08-12 - Android 真机日志首轮定位与重复刷新收口 ✅ 代码完成，待真机复测

- [x] 读取首份 V2 JSON：本地首屏渲染正常，觉察日记 `8–9ms`、我的 `21–28ms`；字体 20 秒超时没有阻塞页面。
- [x] 线上字体文件返回 `200`、CORS 正确、Vercel 缓存命中，大小 `295,564B`；当前失败属于 Android 微信到 Vercel 字体链路超时，不是路径、域名或字体文件缺失。
- [x] 日志发现 43 秒内会员状态请求 9 次（累计约 20.1 秒），记录/选项各 9 次，标注类型/分配各 3 次；三个主页面新增 10 秒后台刷新门控，同页并发和快速往返不再重复请求。
- [x] 会员状态增加账号级并发合并，多个页面同时读取只发一个请求；不增加长期业务缓存，付款后的会员状态仍可及时刷新。
- [x] 远程字体失败后进入 6 小时冷却，回前台不再重复等待 20 秒；界面始终先用系统字体显示。
- [x] 事件环由 120 扩到 240，JSON 导出由最近 100 条扩到 200 条，避免本次最早 25 条启动事件被覆盖。
- [x] 完整小程序自动化测试 `211/211` 通过。
- [ ] 用“预览版/体验版”而非高延迟真机调试链路复测字体；若仍超时，下一步把字体迁到国内 OSS/CDN，而不是继续调整页面代码。
- [ ] 单独打开订单页刷新一次状态，确认本日志中的 `/api/membership/order/status` 单次 `502` 是否仍能复现。

**下一步：** 清缓存并重新编译，先快速往返“今日练习 → 觉察日记 → 我的”两轮，再导出新日志；会员状态、记录和选项请求次数应明显下降。开发者工具显示的 `15512ms` 是手机与真机调试工具之间的往返延迟，会放大调试卡顿，不等于页面本地渲染耗时。

---

## 2026-08-12 - 小程序诊断日志 V2 ✅ 代码完成，待真机采样

- [x] 建立最多 120 条的统一事件时间线：启动、前后台、网络、页面、认证、同步、照片、支付、字体和异常。
- [x] 普通请求记录接口、状态、耗时、Request-ID；3 秒以上标慢，超时记录 timeout 与 errno。
- [x] OSS 照片上传记录字节、MIME、状态和耗时；同步/照片批次/支付记录开始、结束与失败阶段。
- [x] 日志以内存缓冲 + 350ms 延迟落盘，错误/断网立即保存，避免诊断功能拖慢页面。
- [x] 递归脱敏邮箱、密码、验证码、JWT、Bearer、Token、密钥及支付签名；运行日志不再输出完整邮箱。
- [x] 按网页版真源升级运行日志交互：生成后预览完整内容，底部唯一按钮导出 `ashtanga-debug-log-YYYY-MM-DD.json`；真机调起微信文件分享，能力不可用时才降级复制。
- [x] 导出 schema v2 包含小程序版本环境、设备屏幕、安全区、授权、存储、风险标记、摘要与最近 100 条事件。
- [x] 小程序完整自动化测试 `206/206` 通过，JavaScript 语法、JSON 文件名/写入/分享和 UI 回归通过。
- [ ] 重新编译后在 Android 真机完整走一遍冷启动、四入口、同步、照片和支付查询，导出首份 V2 日志验收字段与真实耗时。

**下一步：** 重新编译后在 Android 真机复现问题，再到“我的 → 设置 → 数据管理 → 运行日志 → 导出文件”，把生成的 JSON 分享到文件传输助手或直接发给开发者；开发者工具不支持文件分享时会自动复制完整 JSON。

---

## 2026-08-12 - 真机冷启动超时 / WXSS 兼容 ✅ 代码完成，待真机复测

- [x] 区分无害提示与故障：`SharedArrayBuffer`、HarmonyOS `getSystemInfo`、worker realtime report 均不是启动失败根因。
- [x] 远程字体退出 `App.onLaunch` 关键路径，首屏先用系统字体渲染。
- [x] 冷启动自动查单限制为当前账号 30 分钟内最新 1 笔，避免历史沙箱待确认订单每次启动触发超时。
- [x] 我的页后台查单使用同一限制；旧订单只在订单页按用户操作查询。
- [x] 修复 6 个自定义组件的非法 WXSS 标签/属性/后代标签选择器。
- [x] 请求超时写入脱敏接口地址，后续运行日志可以直接定位失败接口，不再只有 `request:fail timeout`。
- [x] 新增旧订单跳过与超时地址日志测试；小程序完整自动化测试 `200/200` 通过，组件 WXSS 静态扫描无非法选择器。
- [ ] 开发者工具“清缓存并编译”后重新真机调试，确认首屏能启动且控制台不再出现上述组件 WXSS 警告。
- [ ] 若仍有 timeout，导出本轮新的运行日志与发生时间，定位剩余具体接口。

**下一步：** 必须用重新编译后的代码包测试；旧真机调试包不会自动包含本轮启动链路和组件样式修复。

---

## 2026-08-12 - Tab1 / 我的首屏加载性能 ✅ 代码完成，待真机测速

- [x] 定位 3～5 秒“正在准备练习空间 / 正在读取我的数据”并非渲染器慢，而是整页等待云端选项、记录、资料、会员、今日人数和订单补查。
- [x] 今日练习改为先同步读取本机练习类型、当月记录、会员状态和今日人数缓存，立即渲染；云端数据在后台刷新。
- [x] 我的页改为先同步读取账号工作区的全年记录、资料、练习类型和会员缓存，设置按钮立即可用。
- [x] 待确认支付订单恢复改为后台任务，不再排在“我的”资料读取之前；后台到账仍会刷新会员状态。
- [x] 后台刷新失败时保留当前本机界面，不再把有缓存的页面替换成整页错误或清零。
- [x] 新增缓存首屏与非阻塞订单恢复回归；小程序完整自动化测试 `198/198` 通过。
- [ ] 微信开发者工具与 Android 真机分别冷启动、二次进入和四 Tab 往返测速；Tab1 和“我的”应立即出现，不再停留 3～5 秒准备文案。
- [ ] 等后台刷新完成后，确认今日人数、会员期限、热力图和同步状态可无跳页地自动校准。

**下一步：** 重新编译后先连续切换“今日练习 → 我的 → 设置 → 今日练习”两轮；若仍有明显等待，导出运行日志并记录具体入口和秒数，再继续定位字体、图片或单个组件渲染耗时。

---

## 2026-08-12 - Tab1「今日练习」UI 真源统一 ✅ 代码完成，待真机验收

- [x] 页面与窗口背景统一为网页版 `#F9F8F6`。
- [x] 普通未选中练习卡取消半透明白底和绿色描边，恢复“同底色纸面 + 极淡中性边框 + 软阴影”。
- [x] 选中、自定义、会员锁定卡片的渐变、虚线、透明度和阴影按 `PracticeDashboard.tsx` 对齐。
- [x] 练习卡网格横向留白恢复网页版比例；圆形开始按钮的未选中底色、选中阴影和模糊增强同步真源。
- [x] 新增 UI 真源回归断言；小程序完整自动化测试 `196/196` 通过，`git diff --check` 通过。
- [ ] 微信开发者工具重新编译，检查普通/选中/自定义/会员锁定四种状态。
- [ ] Android 真机确认不支持 `backdrop-filter` 时仍保持同色纸面质感，没有重新出现明显白底绿框。

**下一步：** 先完成本轮 Tab1 UI 真机验收；通过后按支付沙箱清单一次性补测年度套餐、取消、中断恢复、重复查单与网页同步。

---

## 2026-08-11 - 小程序虚拟支付闭环 ✅ 代码完成，待沙箱部署与真机付款

- [x] 确认 Pro 会员属于数字化付费功能，会员入口从普通微信支付切换为微信虚拟支付“道具直购”。
- [x] 创建季度 `pro90d` 与年度 `pro365d` 两个道具及 200×200 白底绿字图片；价格分别为 ¥19.8/90 天、¥69.8/365 天。
- [x] 小程序切换为 `wx.requestVirtualPayment`；服务端以虚拟支付 AppKey 和本次微信 `session_key` 生成双签名，客户端不接触密钥。
- [x] 服务端接入 `/xpay/query_order` 和 `/xpay/notify_provide_goods`，复用现有事务函数幂等开通/续费会员。
- [x] 新增虚拟支付订单字段、健康检查、错误码提示、发布密钥扫描与支付专项测试。
- [x] 明确季度/年度都是一次性购买，不开通自动续费会员订阅；Android 沙箱期间不启用苹果 IAP 和平台路径。
- [x] 在 Supabase 执行 `20260811_migrate_payment_orders_to_virtual_pay.sql`。
- [x] 在 Vercel 配置 OfferID、沙箱/现网 AppKey、`WECHAT_VIRTUAL_PAY_ENV=1` 并重新部署。
- [x] Android 开发版已完成首笔沙箱支付，微信支付成功后自动开通/顺延会员，会员剩余天数显示正确。
- [ ] 补测取消支付、另一套餐、网页同步和重复查单不重复履约。
- [x] 支付中断恢复代码完成：订单创建后按账号持久化；小程序重新进入前台及会员页读取前自动补查；已支付/失败/关闭/退款后清理，网络异常继续保留 7 天重试。
- [x] 会员页新增“订单记录”入口：服务端按当前账号返回最近 20 笔订单，展示套餐、金额、时间、状态和订单号；待确认订单支持手动刷新并补发会员。
- [x] 订单列表接口已推送生产 `master` 并由 Vercel 部署；线上匿名检查返回 `401 NOT_AUTHENTICATED`，确认原 404 已消失且鉴权生效。
- [x] 订单入口最终调整为 PRO 大卡片上方的独立满宽小卡片：左侧使用包内订单 SVG、标题与说明，右侧箭头，整卡可点击。
- [x] 沙箱支付订单在订单列表中明确显示“测试订单”；修复“刷新状态”按钮文字受原生行高影响向下偏移的问题，改用 flex 水平垂直居中。
- [ ] Android 真机验证支付中断恢复：付款后立即关闭小程序，再次打开时自动补发会员并显示“会员已到账”。
- [ ] 沙箱通过后发布道具现网版本，将支付环境切换为 `0`；iOS IAP 另开一轮接入。

**下一步：** 接口已经部署并消除 404；重新编译小程序，确认两笔未付款沙箱订单显示“测试订单”、刷新按钮文字居中，然后补测年度套餐、取消支付、中断恢复、网页同步和重复查单。

---

## 2026-08-11 - 小程序体式图片与毛玻璃修复 ✅ 代码完成，待真机复验

- [x] 修复体式库 94 张图片全挂：不再使用真机兼容不稳定的分包本地 WebP，统一生成 720×720 本地 PNG。
- [x] 图片路径从分包绝对路径改为相对页面的 `../../images/`，避免分包资源被当作主包路径解析。
- [x] 二轮修复图片方框：JPG 的固定米白底无法匹配页面从米白到白色的渐变；现改为保留网页版透明通道的调色板 PNG，94 张角落透明度均已验证，图片合计约 1.02 MiB。
- [x] 旧 WebP/JPG 已由同步脚本安全清理；当前 94 张 PNG、0 张 JPG、0 张 WebP。
- [x] 底部四栏和体式库顶部分类栏加入 `backdrop-filter`；Android 不支持模糊时使用 88%/86% 白色兜底，不再透出清晰文字。
- [x] 主包约 1.45 MiB，体式分包约 1.13 MiB；小程序自动化 178/178、包体门禁通过。
- [ ] 开发者工具执行“清缓存并编译”，真机复验五分类图片、详情大图、顶部分类栏和底部四栏遮挡效果。

**下一步：** 如果清缓存后仍有单张图片不显示，记录具体分类和卡片名称；如果 Android 仍不执行 blur，只调整白色兜底透明度，不再依赖设备原生毛玻璃能力。

---

## 2026-08-10 - 小程序体式库同步 ✅ 代码完成，待真机验收

- [x] 以网页版 `components/PosesTab.tsx`、`lib/pose-data.ts` 和三份动作解析数据为唯一真源，同步 5 个分类、94 张正式体式卡片。
- [x] 小程序悬浮导航升级为“今日练习 / 觉察日记 / 体式库 / 我的数据”四入口；体式库使用独立分包，其他三个官方 Tab 保持不变。
- [x] 列表同步顶部分类胶囊、三列正方形图片、中文卡片名和底部安全区，不增加搜索入口。
- [x] 详情同步完整图片、梵文主标题、中文副标题、Vinyasa 总数、凝视点、逐步动作、体位法高亮、停留呼吸、资料说明和上一个/下一个循环切换。
- [x] 94 张素材全部复制到 `weapp/pose-package/images/`，不依赖线上图片；2026-08-11 最终改为兼容真机且保留透明通道的 PNG，分包约 1.13 MiB。
- [x] Noto Serif SC 网络字体升级到 v2，覆盖体式库新增汉字；使用新 URL 避免旧版一年缓存继续命中。
- [x] 新增网页版→小程序同步脚本与 5 项体式库回归；小程序自动化 178/178、发布门禁通过。
- [ ] 微信开发者工具重新编译，真机验收四栏导航、五分类横向切换、94 张图片、长 Vinyasa 详情滚动、前后循环及底部安全区。

**下一步：** 先完成本轮体式库真机 UI 验收；若只发现图片比例、字号或间距问题，按单项返修，不改网页版冻结的数据和排序。

---

## 2026-07-30 - 一序列体式库 ✅ 已完成，进入反馈返修阶段

- [x] 体式库只展示“一序列”，包含拜日 A、拜日 B、站立体式、坐立体式、结束体式五个分类。
- [x] 94 张正式体式卡片已接入 APP：11 + 19 + 18 + 33 + 13。
- [x] 拜日 A/B、站立、坐立、结束体式已接入手册 v2 的名称、Vinyasa 动作解析、呼吸和凝视点。
- [x] 女性 IP 素材、透明/无背景视觉、WebP 资源、详情页和顶部分类栏已完成当前版本收口。
- [x] 搜索入口已移除，顶部分类栏固定在上方，不随列表滚动。
- [ ] 后续根据手机端和用户反馈，按单张图片或单条文案建立返修项；不再整组重做。

**状态文档：** `docs/pose-library/STATUS.md`

---

## 2026-07-31 - 小红书「阿斯汤加体式库」离线体验版 ✅ 第一版开发完成

- [x] 将正式体式库 94 张完成版卡片同步到 `xhs-tool/`，小工具四栏目为拜日 A/B、站立体式、坐立体式和休息体式。
- [x] 详情页接入提示名称、呼吸、凝视点、Vinyasa 总数、逐步动作、体位法高亮和停留呼吸。
- [x] 同步器适配正式库拆分后的站立、坐立和结束体式数据模块，后续继续以正式体式库为唯一资料源。
- [x] 修正透明 WebP 转 JPG 后出现黑底的问题，统一铺网页米白底；94 条体式各使用一张 768px JPG，列表与详情共用，完整离线包约 2.67 MB。
- [x] 375px、390px、430px 移动端、搜索吸顶、四栏目、详情切换、滚动恢复、PC 模拟安全区和离线资源测试通过（12/12）。
- [x] 增加 `npm run serve` 本地手机测试命令，默认端口 `4178`。
- [x] 下载并安装小红书官方 `minitool-zip-builder` skill 到工作区 `.codex/`，按三份 reference 完成 CSP、端能力、跨端和 ZIP 结构校验。
- [x] 修复平台“文件数量超过 200”问题：部署包精简为 120 个文件，不包含 `node_modules`、`.git`、`dist/build` 外层目录或其他无关产物。
- [x] 生成正式部署包 `xhs-tool/release/ashtanga-pose-library-xhs-20260731.zip`，2.46 MB，`index.html` 位于 ZIP 根目录。
- [x] 第一版功能、视觉、离线资源、自动化测试和规范打包全部收口。
- [ ] 上线验收：在小红书开发工具上传正式 ZIP，完成平台模拟器、真机兼容与审核口径验收。

**状态文档：** `xhs-tool/README.md`
...
当前阶段：第四入口体式库代码完成，进入真机 UI 验收；随后执行全量非支付验收。
当前唯一下一步：微信开发者工具重新编译并验收体式库四栏导航、五分类数量、94 张本地图、详情长文滚动、本分类首尾循环和底部安全区。
...
- [x] 底部导航：今日练习 / 觉察日记 / 体式库 / 我的数据；体式库由分包承载
...
- [ ] 微信开发者工具真机确认四入口切换、体式分包、登录导流和统一布局
...
# 2026-08-12 · 超长诊断日志真机文件分享修复

- [x] V3 日志导出失败后不再自动复制超长 JSON；剪贴板和微信普通消息不再作为传递渠道。
- [x] 真机先写入并分享 `ashtanga-debug-log-YYYY-MM-DD.json`。
- [x] 部分 Android 微信若拒绝 `.json` 扩展名，会自动分享同内容的 `.txt` 文件；内容仍是完整、可解析且不截断的 JSON。
- [x] 开发者工具不支持 `wx.shareFileMessage` 时明确提示“请在手机真机分享”，不再误报“已复制完整内容”。
- [x] 文件分享失败原因、运行平台和尝试格式写入诊断事件。
- [x] 修复真机 `shareFileMessage:fail can only be invoked by user TAP gesture`：完整日志和 JSON/TXT 文件在打开弹窗时提前准备，导出按钮点击后同步、立即调用微信文件分享，不再重新采集或等待写盘。
- [x] JSON 分享若失败，TXT 改由用户点击确认按钮后立即调用，第二次尝试同样具备有效点击手势。
- [x] 小程序完整自动化测试 `215/215`、发布包与发布配置检查通过。
- [ ] Android 手机微信中打开体验版/开发版，进入“我的 → 设置 → 数据管理 → 运行日志”，点击底部导出按钮并将文件发送到文件传输助手。

**下一步：** 重新编译后在手机微信打开日志弹窗，等待完整日志生成，再单独点击底部导出按钮。此时按钮不会再触发云端/照片诊断，只会立即调用文件分享；若 JSON 被拒绝，再点击系统弹窗中的“分享 TXT”。

---
## 2026-08-12 · 跨 Tab 后台请求收敛 ✅ 代码完成，待真机日志复验

- [x] 新增 `weapp/services/shared-read-cache.js`：同一账号的练习选项、相同日期范围记录、标注类型和同月标注支持 30 秒跨页面并发合并与新鲜缓存。
- [x] 会员状态使用已有本地 `fetched_at` 做 30 秒短缓存；付款、订单到账和人工查单继续 `{ force: true }` 强制校准，不延迟会员到账。
- [x] 今日练习人数改为 5 分钟缓存；点击“今日人数”仍可强制刷新。
- [x] 待同步队列为空时直接返回，不再产生三组 `pending_operations_start/finish` 空日志；显式同步和照片补传保持原流程。
- [x] Tab Two 和“我的”的手动同步会先清除当前账号共享读取缓存，再重新读取云端。
- [x] 账号工作区增加 `annotation_types_cached`，即使云端标注类型为空也能正确识别为已缓存，不会无限重复请求。
- [x] 小程序完整自动化测试 `218/218` 通过；发布包约 `2.162 MiB`，发布配置门禁通过。
- [ ] Android 重新冷启动并快速切换“今日练习 → 觉察日记 → 我的”两轮，再导出 V3 日志核对请求次数。

**下一步：** 新日志当前会话中，会员状态预期最多 1 次；练习选项预期 1 次；同月记录由今日练习和觉察日记共享 1 次，另保留“我的”全年记录 1 次；空同步事件预期为 0。标注与今日人数的单次 Vercel 请求仍可能受网络/冷启动影响，但不应被快速切 Tab 重复放大。
...
- [x] 运行日志升级为 V3 完整现场包：记录、照片、登录、会员、同步队列、冲突、云端对照和性能时间线均可导出。
- [x] 修复超长日志无法传递：JSON/TXT 文件预生成，用户点击后立即调用微信文件分享；Android 真机已成功导出并收到 222 KB 完整日志。
- [x] 分析首份真实 V3 日志：5 张照片全部健康，本地与云端 2 条记录完全一致，待同步/冲突/孤儿任务均为 0，Pro 剩余 93 天。
- [x] 确认本地首屏并不慢：今日练习 12ms、觉察日记 13ms、我的 35ms；主要延迟来自后台 Vercel 请求和跨 Tab 重复读取。
- [x] 完成跨 Tab 请求治理：会员、选项、同月记录、标注共享短缓存，今日人数缓存 5 分钟，空同步队列直接跳过；付款和手动同步保留强制刷新。
- [x] 完整自动化测试 `218/218`、发布包与发布配置门禁通过；当前包体约 `2.162 MiB`。

### 尚待验证

- [ ] Android 用最新代码冷启动并快速切换“今日练习 → 觉察日记 → 我的”两轮，确认视觉上不再反复校准或等待。
- [ ] 再导出一份 V3 日志：当前会话会员请求应最多 1 次、选项 1 次、当月记录 1 次、全年记录 1 次、空同步事件 0。
- [ ] 单次 Vercel 接口仍可能有 1.5～3.2 秒冷延迟；先确认请求次数下降，再决定是否继续做服务端区域/CDN或接口合并。
- [ ] Noto Serif SC 本轮真机加载约 6.3 秒但已后台成功，不阻塞首屏；如后续仍造成视觉跳变，再单独迁移到国内 OSS/CDN。

**下次第一步：** 不再继续堆功能，先用最新开发版完成上述三 Tab 两轮切换并导出日志。只有确认请求次数达到预期后，才进入下一轮单接口耗时优化。
