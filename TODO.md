...
- [x] 已完成首批 15 页的关键词、素材、来源与 CTA 初步映射
...
- [x] 首批 15 个计划页面已完成，另有 `/tools` 聚合入口
- [x] 5 个公开体式详情已撤掉分步动作内容，只保留名称、图片、序列位置和体式关系
- [x] 首页、知识中心、体式中心、作者页和页脚已统一非教学定位
- [x] 自动测试阻止体式公开页再次出现编号动作步骤和教学化标题
- [x] 55 个测试文件 / 561 项、TypeScript 和轻量 lint 通过
- [x] 联网生产构建通过，39 个路由完成静态生成
- [x] 本地生产服务 19 个公开入口全部返回 200
- [x] 手机、平板、桌面响应式检查通过；修复公开导航在 375px 手机上断字
- [x] 真实页面 canonical、Article / BreadcrumbList JSON-LD 和体式图片加载已核对
- [ ] 人工确认 5 张体式图片使用权、作者简介和二维码真机识别
...
- [x] 小程序计划使用独立 Taro 客户端，复用现有业务 API 和 Supabase 数据，不另建一套微信云业务数据库
- [x] 暂不接微信支付；会员继续使用现有激活码体系
- [x] 不把现有 Next.js/PWA 主工程整体迁移到 Taro
...
- [ ] 注册个人主体小程序，取得 AppID
- [ ] 确定正式名称、简介、头像和可用服务类目
- [ ] 注册后立即提交小程序备案，开发与备案并行
- [ ] 安装微信开发者工具并完成真机预览环境配置
- [ ] 核实个人主体下的会员展示、激活码、联系作者、外部链接及音频能力的审核边界

### 小程序：开发前技术探针

正式迁移页面前，只建立独立 Taro 验证工程，先验证：

- [ ] 小程序真机启动与首屏加载
- [ ] 调用现有业务 API / Supabase 的网络链路与合法域名配置
- [ ] 微信身份与现有邮箱账号的首次绑定及后续无感登录
- [ ] 本地存储、云端同步、重复数据防护和异常恢复
- [ ] 唱诵音频在前台、锁屏和切后台时的实际表现

五项验证通过后，再制定全量功能迁移顺序。不得为赶进度另建一套练习记录或会员数据库。
