# 解耦自动化测试矩阵

状态：`已覆盖` / `部分覆盖` / `缺失` / `阻塞` / `不适用`。

测试层级：L1 纯函数、L2 组件、L3 Hook/适配器集成、L4 隔离浏览器、L5 测试云端冒烟。

## 当前覆盖概览

| 领域 | 当前状态 | 主要缺口 | 计划阶段 |
|---|---|---|---|
| 统计与日记纯函数 | 已覆盖 | 边界与时区补充 | 1 |
| 完成练习/设置组件 | 部分覆盖 | 重复提交、父子弹窗、错误恢复 | 1–2 |
| 练习会话 | 缺失 | 刷新恢复、多次暂停、草稿生命周期 | 2 |
| 音频/唱诵 | 部分覆盖 | Hook 生命周期、缓存失败、模式切换 | 3 |
| Tab/页面编排 | 部分覆盖 | 持久 E2E、动态加载失败、深链接 | 4 |
| 同步 | 部分覆盖 | Hook 编排、远端适配器、失败补偿 | 5 |
| Auth/API/照片 | 部分覆盖 | 服务端输入/未授权/幂等、上传恢复 | 6 |

## 阶段 1：UI 与工具

| 场景 | 层级 | 状态 |
|---|---|---|
| 日期边界、跨月、未来日期、时区 | L1/L2 | 部分覆盖 |
| 时长 0 秒、秒数、整分钟、超长练习 | L1 | 缺失 |
| 自定义选项空值、重名、上限、重复点击 | L2 | 缺失 |
| 免费/Pro 色阶与锁定选项 | L1/L2 | 部分覆盖 |
| 编辑、删除、取消删除、弹窗关闭 | L2 | 部分覆盖 |
| 子弹窗打开时父弹窗不关闭 | L2 | 缺失 |
| 键盘与禁用状态 | L2 | 缺失 |

## 阶段 2：练习会话

| 场景 | 层级 | 状态 |
|---|---|---|
| 开始、暂停、恢复、结束、保存、放弃 | L3/L4 | 部分覆盖 |
| 连续开始/保存幂等 | L3 | 缺失 |
| 多次暂停累计 | L1/L3 | 缺失 |
| 刷新、后台、锁屏恢复 | L3/L4 | 缺失 |
| LocalStorage 缺失/损坏/部分字段 | L3 | 缺失 |
| 0 分钟与短时确认 | L2/L3 | 部分覆盖 |
| 保存失败恢复与重试 | L3 | 缺失 |
| 草稿创建、转正、删除失败 | L3 | 缺失 |
| 跨天、设备时间变化、负数保护 | L1/L3 | 缺失 |
| 完成后切换日记 | L4 | 已覆盖（临时回归） |

## 阶段 3：媒体

| 场景 | 层级 | 状态 |
|---|---|---|
| 网络加载、缓存命中/失败、网络失败 | L3 | 部分覆盖 |
| 加载中重复点击 | L3 | 缺失 |
| 播放、暂停、恢复、快进/后退边界 | L1/L3 | 部分覆盖 |
| 结束复位与卸载资源释放 | L3 | 部分覆盖 |
| 后台恢复进度 | L3/L4 | 缺失 |
| 唱诵倒计时、跳过、取消、结束 | L3 | 缺失 |
| 唱诵结束只启动一次练习 | L3 | 缺失 |
| 音频失败不阻塞普通计时 | L3/L4 | 缺失 |
| 媒体模式切换不串状态 | L3 | 缺失 |

## 阶段 4：页面与动态加载

| 场景 | 层级 | 状态 |
|---|---|---|
| 四个 Tab 双向/快速切换 | L4 | 部分覆盖 |
| 动态模块 loading/success/error | L2/L4 | 缺失 |
| Tab 滚动与内部状态保持 | L4 | 缺失 |
| 弹窗打开隐藏导航、关闭恢复 | L4 | 部分覆盖 |
| URL 参数、返回、刷新、深链接 | L4 | 缺失 |
| 日记 CRUD、分享、月份切换 | L2/L4 | 部分覆盖 |
| 统计空态/历史数据/会员入口 | L2/L4 | 部分覆盖 |
| 设置四分区与数据管理 | L2/L4 | 部分覆盖 |
| 导入合法/非法、导出、三步清空 | L2/L4 | 缺失 |
| 移动端遮挡与溢出 | L4 | 部分覆盖 |

## 阶段 5：同步

| 场景组 | 层级 | 状态 |
|---|---|---|
| 本地/云端空、仅一端新增或更新 | L1/L3 | 部分覆盖 |
| 两端修改不同/相同记录 | L1/L3 | 部分覆盖 |
| remote/local/merge 策略 | L1/L3 | 部分覆盖 |
| profile/options/records 独立变化 | L1/L3 | 部分覆盖 |
| 字段映射与照片四种输入 | L1 | 部分覆盖 |
| 草稿、软删除、孤立记录 | L1/L3 | 部分覆盖 |
| 1000 条限制与排序 | L1/L3 | 缺失 |
| 超时、重试、部分失败恢复 | L3 | 缺失 |
| 重复/并发同步锁 | L3 | 缺失 |
| 损坏本地数据和异常远端响应 | L3 | 缺失 |
| 用户隔离与未登录禁止上传 | L3/L5 | 缺失 |
| 删除的本地成功/云端失败补偿 | L3/L5 | 缺失 |
| 日志数量、大小、敏感信息过滤 | L1/L3 | 部分覆盖 |
| 测试账号真实上传/下载/冲突 | L5 | 阻塞（缺测试账号） |

## 阶段 6：跨模块

| 场景 | 层级 | 状态 |
|---|---|---|
| 登录/注册/忘记密码状态与错误 | L2/L3 | 缺失 |
| 免费/Pro 权限组合与到期降级 | L1/L2 | 部分覆盖 |
| 照片上传/删除失败恢复与上限 | L2/L3 | 部分覆盖 |
| 旧版本导入兼容 | L1/L3 | 缺失 |
| 教程记录与真实记录隔离 | L1/L3 | 缺失 |
| API 输入、未授权、异常、幂等 | L3 | 缺失 |
| 无障碍名称、键盘、焦点恢复 | L2/L4 | 缺失 |

## 规则

- 每次修复回归必须留下永久测试。
- 新模块所有公共操作和错误分支必须有测试。
- 状态机每条合法转换必须覆盖。
- 无法自动化的项目必须记录原因、风险和人工验证方法。
- 测试数量不是完成标准，矩阵中的高风险缺口清零才是。

