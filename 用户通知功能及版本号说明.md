...
Tab2 日历 header 左侧，位于云同步按钮和上月按钮之间：

```
[云同步] [公告(红/绿状态点)] [◀上月]  2026年4月  [下月▶] [铅笔] [+]
```
...
1. **首次访问**: `readInviteVersion = ''` → 状态点显示红色
2. **用户点击公告图标**: 保存 `INVITE_VERSION` 到 localStorage → 状态点变绿
3. **更新文案并部署**: 版本号改变（如 `v1` → `v2`）
4. **用户刷新**: `readInviteVersion = 'v1'` ≠ `INVITE_VERSION = 'v2'` → 状态点变红 ✅
